...
step "Building and deploying the Pattern 3b echo MCP"
docker build -t pattern-3b-echo-mcp:latest \
  "${REPO_ROOT}/showcases/mcp_x_idjag/components/echo-mcp"
kind load docker-image pattern-3b-echo-mcp:latest --name "${CLUSTER_NAME}"
...
kubectl -n "${PATTERN_NAMESPACE}" rollout status deploy/pattern-3b-echo-mcp --timeout=180s
ok "Pattern 3b echo MCP ready"
