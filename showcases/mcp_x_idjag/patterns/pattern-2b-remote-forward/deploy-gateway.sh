...
# cluster (e.g. Pattern 3a's Envoy Gateway) - re-applying --server-side is
# safe and just ensures the CRDs are at least this version.
...
step "Applying Pattern 2b Gateway"
...
if ! kubectl wait --for=condition=Programmed gateway/pattern-2b-gateway \
  -n "${PATTERN_NAMESPACE}" --timeout=180s; then
  kubectl -n "${PATTERN_NAMESPACE}" get gateway pattern-2b-gateway -o yaml || true
  kubectl get svc -A -l gateway.networking.k8s.io/gateway-name=pattern-2b-gateway || true
  fatal "Pattern 2b Gateway was not programmed; inspect the status above"
fi

ok "Pattern 2b Gateway ready (agentgateway control plane installed; routes/policies applied separately)"
