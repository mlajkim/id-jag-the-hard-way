...
```sh
mkdir -p ./api_sever/mcp/certs

cp ./keys/api-mcp.key ./api_sever/mcp/certs/api-mcp.key
cp ./keys/api-mcp.crt ./api_sever/mcp/certs/api-mcp.crt
```

Also copy the CA certificate of Athenz, that is generated by default when you did the athenz distribution:

```sh
cp ./athenz_dist/certs/ca.cert.pem ./api_sever/mcp/certs/ca.crt
```

Then run the server:

```sh
make -C api_sever mcp-local PORT=8101
