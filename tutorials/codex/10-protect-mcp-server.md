...
Use the shared chapter's `curl` commands to see both failures independently of the client interface. MCP access now passes, but Athenz rejects the downstream token exchange. In the next chapter, you will grant the service the missing exchange permissions. Next: [Token Exchange — Codex](./11-token-exchange.md)
