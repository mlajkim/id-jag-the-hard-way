...
The AI client can still connect and discover tools. Runtime Proxy rejects tool calls without a token. The API-audience token configured in Open WebUI in the previous chapter also fails the proxy's MCP audience check; the next steps issue an MCP-audience token.
