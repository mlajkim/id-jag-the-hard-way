...
IDTHW Hub does not generate a user credential in the browser session. Its `idthw-hub.central-controller` workload certificate stays server-side and currently uses the operator's ZMS admin authority to provision a newly created Hub-managed MCP server. The separate Keycloak client ID is `idthw-hub.ui`. During that server-side creation transaction, the Hub applies the custom `mcp_hub_managed_access` template for the server route, applies the custom `mcp_exchange_helpers` template for each configured downstream role, and applies the built-in `zts_instance_launch_provider` template for service-certificate/Copper Argos enrollment. The downstream direct-access role must already exist in its resource-owner domain. The Hub also generates a dedicated service key, registers its public key on the selected Athenz service under the per-server key ID `idthw-hub-<mcp-key>`, and stores the private key in a server-specific Kubernetes bootstrap Secret. Live MCP Hub tool discovery does not mint an Athenz access token. MCP Gateway performs ID-JAG exchange only when an authenticated client invokes a protected MCP method such as `tools/call`.
