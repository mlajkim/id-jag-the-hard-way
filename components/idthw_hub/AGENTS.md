...
- `/api/mcp-cache-status` keeps an empty Hub access-token-cache field for compatibility and aggregates sanitized MCP Gateway OAuth-session metadata with ID-token and opaque-session expiry plus separate access-token and ID-JAG cache summaries from `MCP_HUB_GATEWAY_STATUS_URL`. Both access-token and ID-JAG entries include only audiences, scope, cache time, expiry, and status. It must never expose token or session credential values.
