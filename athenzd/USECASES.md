...
1. Ensures the `home.<*user*>` domain and its `local.athenzd` service exist in **ZMS**, creating them if missing.
   - athenzd calls ZMS with the ID token as the credential; ZMS's `OIDCAuthority` authenticates the caller as `user.<*user*>`.
   - The user self-creates their own `home.<*user*>` domain and the `local.athenzd` service inside it.
