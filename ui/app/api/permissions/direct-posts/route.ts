...
const DIRECT_POSTS_POLICY = {
  domain: "api",
  policyName: "docs-poster_post_docs",
...
function requestZms(method: "GET" | "PUT" | "DELETE", body?: string): Promise<ZmsResponse> {
  ...
  const url = new URL(`${zms}/domain/${DIRECT_POSTS_POLICY.domain}/policy/${DIRECT_POSTS_POLICY.policyName}`);
...
function policyBody() {
  return JSON.stringify({
    name: `${DIRECT_POSTS_POLICY.domain}:policy.${DIRECT_POSTS_POLICY.policyName}`,
    assertions: [
      {
        role: DIRECT_POSTS_POLICY.role,
        resource: DIRECT_POSTS_POLICY.resource,
        action: DIRECT_POSTS_POLICY.action,
      },
    ],
  });
}
