...
const DIRECT_DELETES_POLICY = {
  domain: "api",
  policyName: "docs-deleter_delete_docs",
...
function requestZms(method: "GET" | "PUT" | "DELETE", body?: string): Promise<ZmsResponse> {
  ...
  const url = new URL(`${zms}/domain/${DIRECT_DELETES_POLICY.domain}/policy/${DIRECT_DELETES_POLICY.policyName}`);
...
function policyBody() {
  return JSON.stringify({
    name: `${DIRECT_DELETES_POLICY.domain}:policy.${DIRECT_DELETES_POLICY.policyName}`,
    assertions: [
      {
        role: DIRECT_DELETES_POLICY.role,
        resource: DIRECT_DELETES_POLICY.resource,
        action: DIRECT_DELETES_POLICY.action,
      },
    ],
  });
}
