...
- [Step 7. Capture the mcp access token for the final exchange](#step-7-capture-the-mcp-access-token-for-the-final-exchange)
...
The access token returned by this exchange is the MCP-layer subject token for the next exchange. In Step 7, the same exchange is run with `--token-only` so that token can be captured as `_mcp_at`.
...
## Step 7. Capture the mcp access token for the final exchange

Capture the successful mcp-hub to mcp exchange as `_mcp_at`. This is the access token generated by the previous exchange shape, and it becomes the subject token for the final `api.api-mcp` to API exchange.

This repeats the Step 6 command with `--token-only` so the variable receives only the raw exchanged token:
...
After `_mcp_at` is captured, the final hop is for `api.api-mcp` to exchange that MCP access token into the final API role `docs-getter`. This works because `_mcp_at` still contains `docs-getter`.
