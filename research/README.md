...
- Expected command output should be shown as a commented `sh` block. Keep the status lines (`#   · ...`, `#   ✔ ...`, `#   ✘ ...`) and show only stable, relevant JSON claims. Preserve exact error `code` and `message` fields. For decoded JWT headers, keep the `kid` field visible as `"kid": "<key-id>"` so the token shape is clear. Redact omitted or dynamic payload fields with `#   ...`; do not include raw JWTs, long tokens, `exp`, `iat`, `jti`, `sid`, or other run-specific values unless the field itself is the subject of the research.
